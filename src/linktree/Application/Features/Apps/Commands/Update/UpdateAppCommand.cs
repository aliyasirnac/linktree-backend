...
public class UpdateAppCommand : IRequest<UpdatedAppResponse>, ISecuredRequest, ICacheRemoverRequest, ILoggableRequest,
    ITransactionalRequest
{
    ...
    public class UpdateAppCommandHandler : IRequestHandler<UpdateAppCommand, UpdatedAppResponse>
    {
        ...
        public UpdateAppCommandHandler(IMapper mapper, IAppRepository appRepository,
            AppBusinessRules appBusinessRules, ImageServiceBase imageService)
        {
            _mapper = mapper;
        ...
        {
            App? app = await _appRepository.GetAsync(predicate: a => a.Id == request.Id,
                cancellationToken: cancellationToken);
            await _appBusinessRules.AppShouldExistWhenSelected(app);
            app = _mapper.Map(request, app);
            if (app?.ImageUrl != null) await _imageService.UpdateAsync(request.ImageUrl, app.ImageUrl);
